...
app.post('/api/contact', async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO feedback (session_id, user_email, category, message, page) VALUES ($1,$2,$3,$4,$5)`,
      ['contact_form', email, source || 'contact', `${name ? name + ': ' : ''}${company ? '['+company+'] ' : ''}${message}`, source || 'contact']
    );
    res.json({ ok: true });
  } catch(e) {
    console.error(e);
